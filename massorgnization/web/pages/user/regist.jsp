		...
		<script type="text/javascript">
			...
			$(function () {
				...
				$("#username").blur(function () {
					...
					$.getJSON("http://localhost:8080/book2/userServlet","action=ajaxExistsUsername&username=" + username,function (data) {
						if(data.existsUsername){
							$("span.errorMsg").text("用户学号已注册！");
						}else{
							$("span.errorMsg").text("用户学号可用！");
						}
				...
				$("#code_img").click(function () {
					// 在ie、火狐中浏览器为使下一次访问更快，就会给访问过的地址=资源的形式缓存起来
					// 我们就达不到刷新图片的作用。所以我们要跳过浏览器的缓存，就需要在地址后面加一个
					// 参数，让每次访问的地址不同即可，而时间戳是一个很好的选择(new Date())。
							...
							<div class="form">
								<form action="userServlet" method="post">
									...
									<input class="itxt" type="text" placeholder="请输入学号"
										   value="${requestScope.studentnumber}"
										   autocomplete="off" tabindex="1" name="studentnumber" id="username" />
									<br />
									<br />
									<label>用户密码：</label>
